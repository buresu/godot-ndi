...
NdiInputStream::NdiInputStream() : Object() {
    _mutex = memnew(Mutex);
}
...
void NdiInputStream::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("get_frame_count"), &NdiInputStream::get_frame_count);
    ClassDB::bind_method(D_METHOD("get_frame"), &NdiInputStream::get_frame);
    ClassDB::bind_method(D_METHOD("set_source_name", "name"), &NdiInputStream::set_source_name);
    ClassDB::bind_method(D_METHOD("set_bandwidth", "bandwidth"), &NdiInputStream::set_bandwidth);
    ClassDB::bind_method(D_METHOD("_thread_function"), &NdiInputStream::_thread_function);
...
void NdiInputStream::_thread_function() {
    while (!_should_exit) {
        ...
        recv_create_desc.bandwidth = (_bandwidth == HIGHEST) ? NDIlib_recv_bandwidth_highest : NDIlib_recv_bandwidth_lowest;
        ...
        if (_ndi_receiver) {
            ...
            while (!_should_exit && continue_receiving) {
                ...
                switch (NDIlib_recv_capture_v3(_ndi_receiver, &video_frame, &audio_frame, nullptr, 5000)) {
                    case NDIlib_frame_type_video: {
                        PackedByteArray image_data;
                        image_data.resize(video_frame.xres * video_frame.yres * 4);
                        memcpy(image_data.ptrw(), video_frame.p_data, image_data.size());

                        Ref<Image> new_frame = Image::create_from_data(video_frame.xres, video_frame.yres, false, Image::FORMAT_RGBA8, image_data);
